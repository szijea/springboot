...
public class StockInController {
    ...
    public ResponseEntity<?> createStockIn(@RequestBody StockIn stockIn) {
        try {
            ...
            // 更新库存：按 medicineId + batchNumber 合并库存
            if (stockIn.getItems() != null) {
                for (StockInItem item : stockIn.getItems()) {
                    try {
                        String medId = item.getMedicine() != null ? item.getMedicine().getMedicineId() : null;
                        String batch = item.getBatchNumber() != null ? item.getBatchNumber() : "DEFAULT_BATCH";
                        Integer qty = item.getQuantity() != null ? item.getQuantity() : 0;
                        java.time.LocalDate expiry = item.getExpiryDate();

                        if (medId != null) {
                            // 查找是否存在相同 medicineId + batch
                            java.util.List<com.pharmacy.entity.Inventory> invs = inventoryRepository.findByMedicineId(medId);
                            com.pharmacy.entity.Inventory matched = null;
                            for (com.pharmacy.entity.Inventory inv : invs) {
                                if (batch.equals(inv.getBatchNo())) {
                                    matched = inv;
                                    break;
                                }
                            }
                            ...
                            if (matched != null) {
                                ...
                                newInv.setPurchasePrice(item.getUnitPrice() != null ? new java.math.BigDecimal(item.getUnitPrice()) : null);
                                inventoryRepository.save(newInv);
                            }
                        }
                    } catch (Exception invEx) {
                        System.err.println("更新库存记录失败: " + invEx.getMessage());
